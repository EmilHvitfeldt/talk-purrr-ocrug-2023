...
::: fragment
```{r}
map(mtcars, \(x) mean(x, rm.na = TRUE))
```
:::
:::
:::
...
These two statements ar almost identical
...
Having models that sometimes errors are a living nightmare, using `safely()` lets to run everything and diagnose later
...
Web scraping typically cals for delays between fetching, we handle this with `slowly()`
